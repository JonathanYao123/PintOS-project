...
kill (struct intr_frame *f)
{
  ...
  switch (f->cs)
    ...
    case SEL_UCSEG:
      ...
      intr_dump_frame (f);
      exit (EXIT_FAILURE); /* update the exit status before exiting */
...
page_fault (struct intr_frame *f)
{
   ...
   *     not expect any data at the address it was trying to access, or if
   *     the page lies within kernel virtual memory, or if the access is an
   *     attempt to write to a read-only page, then the access is invalid. Any
   *     invalid access terminates the process and thereby frees all of its
   *     resources.
   *
